...
build_w_pot <- function(post, K, N_pot, d_pot, run){
  ...
  if (kernel=='gaussian'){
    ...
    one_b = run$one_b
    if (one_b){
      b = rep(median(post[,1,which(par_names == 'b')]), K)
    } else {
      b = apply(post[,1,which(par_names == 'b')],2,median)
    }
    for (k in 1:K)
...
dispersal_decay <- function(post, dmat, sum_w, radius, run, taxa){
  ...
  phi   = colMeans(post[,1,which(par_names == 'phi')])
  if (one_gamma){
    gamma = rep(median(post[,1,which(par_names == 'gamma')]), K)
  } else {
    gamma = apply(post[,1,which(par_names == 'gamma')],2,median)
  }
